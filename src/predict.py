"""Run inference for DETR digit detection and export COCO-format predictions."""
...
class InferenceCollator:
    """Batch collator for inference-only image batches."""

    def __init__(self, min_size, max_size):
        """Initialize image processor with configurable resize limits."""
    ...
    def __call__(self, batch):
        """Convert raw PIL images into model-ready tensors."""
...
def predict():
    """CLI entrypoint for generating `pred.json` style detection outputs."""
    ...
    parser.add_argument("--data", "-d", type=str, required=True, help="Test images directory")
    parser.add_argument(
        "--output",
        "-o",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="Confidence threshold for exported detections",
    )
