...
**Command:**
```bash
python src/train.py --epochs 50 --batch-size 8 --lr 1e-4 --lr-backbone 1e-5 --min-size 800 --max-size 1333
```
**Rationale:** 
Establishes a solid control group using standard DETR defaults (Cosine learning rate scheduler, 1e-5 backbone LR, 50 epochs). We need this baseline to measure whether our subsequent architectural "hacks" actually improve the model or just introduce instability.

### 2. Attacking the "Small Object" Problem (Upscaling)
**Command:**
```bash
python src/train.py --epochs 50 --batch-size 4 --min-size 1000 --max-size 1500
```
**Rationale:** 
...
**Command:**
```bash
python src/train.py --epochs 100 --batch-size 8 --weight-decay 2e-4 
```
**Rationale:** 
The Hungarian Matcher inside DETR takes significantly longer to stabilize compared to traditional CNN anchors like YOLO. 50 epochs might be cutting the learning off just as the bipartite matching clicks. By doubling the epochs to 100, we allow full convergence. However, to prevent the model from memorizing the 30k training images over this extended duration, we double the `weight-decay` (from `1e-4` to `2e-4`).

### 4. Backbone Preservation (Extreme Differential LR)
**Command:**
```bash
python src/train.py --epochs 50 --batch-size 8 --lr 1e-4 --lr-backbone 5e-6
```
**Rationale:** 
...
**Command:**
```bash
python src/train.py --epochs 100 --batch-size 4 --min-size 1000 --max-size 1333 --weight-decay 1.5e-4 --lr-backbone 5e-6
```
**Rationale:** 
